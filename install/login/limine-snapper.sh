if command -v limine &>/dev/null; then
  ...
  sudo tee /etc/mkinitcpio.conf.d/zarchlinux_hooks.conf <<EOF >/dev/null
...
EOF
  ...
  sudo cp $ZARCHLINUX_PATH/default/limine/default.conf /etc/default/limine
  ...
  sudo cp $ZARCHLINUX_PATH/default/limine/limine.conf /boot/limine.conf

  # Match Snapper configs if not installing from the ISO
  if [[ -z ${ZARCHLINUX_CHROOT_INSTALL:-} ]]; then
