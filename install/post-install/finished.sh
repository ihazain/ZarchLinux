...
clear
echo
tte -i ~/.local/share/zarchlinux/logo.txt --canvas-width 0 --anchor-text c --frame-rate 920 laseretch
echo

# Display installation time if available
if [[ -f $ZARCHLINUX_INSTALL_LOG_FILE ]] && grep -q "Total:" "$ZARCHLINUX_INSTALL_LOG_FILE" 2>/dev/null; then
  echo
  TOTAL_TIME=$(tail -n 20 "$ZARCHLINUX_INSTALL_LOG_FILE" | grep "^Total:" | sed 's/^Total:[[:space:]]*//')
