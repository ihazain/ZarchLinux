...
zarchlinux-refresh-config uwsm/default
zarchlinux-refresh-config uwsm/env
zarchlinux-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, zarchlinux-launch-editor' ~/.config/hypr/bindings.conf
fi
