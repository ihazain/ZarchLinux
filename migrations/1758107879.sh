...
for pkg in "${PACKAGES[@]}"; do
  if ! zarchlinux-pkg-present "$pkg"; then
...
if $NEEDS_MIGRATION; then
  ...
  zarchlinux-pkg-drop walker-bin walker-bin-debug

  zarch-pkg-add "${PACKAGES[@]}"

  source $ZARCHLINUX_PATH/install/config/walker-elephant.sh

  rm -rf ~/.config/walker/themes

  zarchlinux-refresh-config walker/config.toml
  zarchlinux-refresh-config elephant/calc.toml
  zarchlinux-refresh-config elephant/desktopapplications.toml
fi
