...
# Users can still opt-in to Cloudflare DNS using: zarchlinux-setup-dns cloudflare
...
if [[ -f /etc/systemd/resolved.conf ]]; then
  ...
  if [[ -f /etc/systemd/network/99-zarchlinux-dns.network ]]; then
    sudo rm -f /etc/systemd/network/99-zarchlinux-dns.network
  ...
  echo "To use Cloudflare DNS, run: zarchlinux-setup-dns Cloudflare"
fi